...
def uniformCostSearch(problem: SearchProblem):
    ...
    initial_node = problem.getStartState()
    frontier.push((initial_node,[],0),0)
    visited = []
    
    while not frontier.isEmpty():
        
        current_node, path, costs= frontier.pop()
        if not current_node in visited:
            visited.append(current_node)
            if problem.isGoalState(current_node):
                return path
            for node,action,cost in problem.getSuccessors(current_node):
                if not node in visited:
                    frontier.push((node, path + [action],cost + costs),cost + costs)
    util.raiseNotDefined()
...
def aStarSearch(problem: SearchProblem, heuristic=nullHeuristic):
    ...
    while not frontier.isEmpty():
        ...
        if not current_node in visited:
            ...
            for node,action,cost in problem.getSuccessors(current_node):
                if not node in visited:
                    h = heuristic(node,problem) + cost + costs
                    frontier.push((node, path + [action],cost + costs),h)
    util.raiseNotDefined()
